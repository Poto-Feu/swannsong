...
static void display_strings(std::vector<std::string> const& p_strings,
        ...
        unsigned int max_lines, pstrings::ps_data_ptr const& pstrings_data)
{
...
void dialogbox::display(const std::string *title,
        ...
        pstrings::ps_data_ptr const& pstrings_data)
{
    ...
    while(!screen_exited) {
        ...
        if(title) {
            ...
            current_max_content_lines = max_content_lines + title_line
                - content_start_line;
        }
        ...
        if(p_strings) {
            ...
            for(auto const& it : *p_strings) {
                const std::vector<std::string> itVec =
                    pcurses::divide_string_into_lines(it);
                vec_lines.insert(vec_lines.end(), itVec.begin(), itVec.end());
            }

            strings_n_lines = vec_lines.size();
            display_strings(vec_lines, strings_display_offset,
                    content_start_line, current_max_content_lines,
                    pstrings_data);
        }
        ...
        while(!is_key_pressed) {
            ...
            if(key_pressed == KEY_UP && strings_display_offset > 0
                    && p_strings) {
                strings_display_offset -= 1;
                is_key_pressed = true;
            } else if(key_pressed == KEY_DOWN && p_strings) {
                if(strings_n_lines - strings_display_offset
                        > current_max_content_lines) {
